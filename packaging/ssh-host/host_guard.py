#!/usr/bin/env python3
"""Root-owned, single-operation SSH host guard; this is not an approval broker.
...
class Ledger:
    def __init__(self, path):
        ...
        # The host CLI runs as root with umask 077. No secrets or probe output
...
class Guard:
    def __init__(self, ledger, host, principal, uid, *, probe_argv=PROBE_ARGV, gid=None, broker=None):
        self.ledger = ledger
    ...
    def run(self, identifier):
        ...
        try:
            ...
            contract = self.broker.check_health(authority[0]) if self.broker else None
            cancellation = self.ledger.cancellation(claim)
            if cancellation:
                status, code = cancellation, cancellation
            else:
                process = subprocess.Popen(
                    self.probe_argv, stdin=subprocess.PIPE if contract else subprocess.DEVNULL, stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT, cwd="/", env={"PATH": "/usr/bin:/bin", "LANG": "C", "LC_ALL": "C"},
                    start_new_session=True, close_fds=True, **identity,
                )
                if contract:
                    process.stdin.write(json.dumps(contract, sort_keys=True).encode())
                    process.stdin.close()
                    ...
                    if eof and returncode is not None:
                        ...
                        if returncode != 0:
                            ...
                            try:
                                ...
                                if contract and result != {"service": contract["service"], "status": "ok", "version": contract["version"]}:
                                    raise ValueError("health result outside approved contract")
                                if self.broker:
                                    self.broker.check_health(authority[0])
...
def validate_ledger_path(path):
    path = Path(path)
    if not path.is_absolute() or ".." in path.parts:
        raise ValueError("absolute ledger path required")
    for parent in reversed(path.parents):
        metadata = parent.lstat()
        if not stat.S_ISDIR(metadata.st_mode) or metadata.st_uid != 0 or metadata.st_mode & 0o022:
            raise ValueError("ledger directory requires root custody")
    if os.path.lexists(path):
        metadata = path.lstat()
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_uid != 0 or metadata.st_mode & 0o077:
            raise ValueError("ledger must be root owned mode 0600")
...
def main(argv=None):
    ...
    try:
        ...
        if os.geteuid() != 0:
            raise ValueError("host control plane requires root")
        os.umask(0o077)
        validate_ledger_path(args.db)
        ...
        if args.command == "grant":
            ...
            if account.pw_uid != args.uid or account.pw_uid <= 0:
                raise ValueError("operation UID must match configured SSH login")
            broker = module.BrokerControl(ledger, config, receipt_key, args.host, args.principal)
            serve(Guard(ledger, args.host, args.principal, args.uid, gid=account.pw_gid, broker=broker), args.socket)
        return 0
    except (ValueError, OSError, sqlite3.Error):
