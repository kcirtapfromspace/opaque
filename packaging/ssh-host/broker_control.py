...
PINNED_FIELDS = {
    ...
    "vault_ca_sha256", "vault_token_ref", "health_contract",
}
...
CONFIG_FIELDS = PINNED_FIELDS | {
    "broker_public_key_hex", "receipt_private_key_path", "max_session_secs", "tls_cert_path", "tls_key_path", "control_listen_host", "control_listen_port",
}
...
def validate_action(action):
    ...
    host_config.validate_contract(action["health_contract"])
...
protected_bytes = host_config.protected_bytes
...
def load_config(path=CONFIG_PATH):
    ...
    canonical_ip(config["control_listen_host"])
    if type(config["control_listen_port"]) is not int or not 1 <= config["control_listen_port"] <= 65535:
        raise ValueError("invalid control listener port")
    principal = protected_bytes("/etc/opaque-ssh/principals", 128).decode("ascii").strip()
    if principal != config["principal"]:
        raise ValueError("configured principal does not match SSH authentication hook")
    account = pwd.getpwnam(config["login_user"])
    if account.pw_uid <= 0:
        raise ValueError("SSH login must be unprivileged")
    ...
    host_config.validate_contract(config["health_contract"])
    if config["health_contract"] != host_config.load_health():
        raise ValueError("local health configuration does not match profile")
...
class BrokerControl:
    def __init__(self, ledger, config, receipt_key, host, principal, *, clock=time.time):
        ...
        if config["principal"] != principal or config["login_user"] == "root":
    ...
    def check_health(self, manifest):
        contract = host_config.load_health()
        if manifest.get("health_contract") != self.config["health_contract"] or contract != self.config["health_contract"]:
...
def main():
    ...
    module.validate_ledger_path(args.db)
    control = BrokerControl(module.Ledger(args.db), config, receipt_key, args.host, args.principal)
    server = ControlServer((config["control_listen_host"], config["control_listen_port"]), control)
