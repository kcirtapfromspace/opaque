...
class GitHub:
    def get(self, path):
        ...
        if not (path == f"repos/{REPO}" or path.startswith((f"repos/{REPO}/", "users/kcirtapfromspace/packages/"))):
...
def load_contract(manifest_path, workflow_path):
    try:
        ...
        if document["schema_version"] != 2 or not isinstance(actions, list) or len(actions) != 1:
            raise ValueError
        action = actions[0]
...
def inspect_contract(action, workflow, api):
    ...
    status, branch = get_object(f"{prefix}/branches/{BRANCH}")
    commit_value = branch.get("commit")
    commit = commit_value.get("sha", "") if isinstance(commit_value, dict) else ""
    branch_valid = status == 200 and branch.get("name") == BRANCH and isinstance(commit, str) and bool(re.fullmatch(r"[0-9a-f]{40}", commit))
    ...
    status, permissions = get_object(f"{prefix}/actions/permissions")
    ...
    status, protection = get_object(f"{prefix}/branches/{BRANCH}/protection")
    if status == 200:
    ...
    status, metadata = get_object(f"{prefix}/actions/workflows/{WORKFLOW_PATH.rsplit('/', 1)[1]}")
    ...
    def content_matches(ref, name):
        status, content = get_object(f"{prefix}/contents/{WORKFLOW_PATH}?ref={quote(ref, safe='')}")
        matched = False
        observed_hash = None
    ...
    status, environment = get_object(f"{prefix}/environments/staging")
    if status == 200 and environment.get("name") == "staging":
        ...
        if policy.get("custom_branch_policies") is True:
            policy_status, policies = get_object(f"{prefix}/environments/staging/deployment-branch-policies?per_page=100")
    ...
    status, package = get_object(package_path)
