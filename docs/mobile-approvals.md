...
2. **Opaque device key (recommended for v1 mobile app)**:
   - The Opaque iOS app creates a device-bound signing key in Secure Enclave.
...
4. iOS app generates a Secure Enclave signing key and sends:
   ...
   - `pairing_code`
5. `opaqued` verifies `pairing_code` + TTL and stores the device public key.
...
6. `opaqued` verifies signature using stored `device_pubkey`, checks TTL, marks approved and issues an approval lease.
