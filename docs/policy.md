...
Note: `secret_names` enforcement currently depends on `secret_ref_names` being populated on the request. Some convenience wrappers (e.g., `exec`, `github`, `onepassword`) do not currently provide reliable `secret_ref_names`, so `secret_names` should be treated as best-effort until that is fixed.
