...
| **HashiCorp Vault provider connector** | Fetch secrets from Vault KV, dynamic secrets (DB, AWS) | ✅ **Done** — [`vault:` resolver (KV + dynamic field reads, proactive renewal, cache lifecycle)](../crates/opaqued/src/vault/resolve.rs), [Vault client (lease metadata + renew + revoke API)](../crates/opaqued/src/vault/client.rs), [resolver wiring](../crates/opaqued/src/sandbox/resolve.rs), [current scope docs](vault.md#current-scope). | Large |
...
| **Audit retention policies** | Configurable retention with automatic purge | ✅ **Done** — [configurable `audit_retention_days` (default 90)](../crates/opaqued/src/main.rs) + [startup cleanup and periodic runtime purge](../crates/opaque-core/src/audit.rs). Parquet lifecycle remains tracked by the separate [Arrow/Parquet audit export](#v4-analytics--enterprise-features) item. | Medium |
