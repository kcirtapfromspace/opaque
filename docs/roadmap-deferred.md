...
| **HashiCorp Vault provider connector** | Fetch secrets from Vault KV, dynamic secrets (DB, AWS) | **Partially shipped** — `vault:` refs resolve KV + dynamic engine fields for existing write-only operations, with lease-duration caching. Lease renewal/revoke lifecycle remains deferred. | Large |
