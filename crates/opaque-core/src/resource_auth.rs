...
pub struct Admission {
    ...
    pub subject: String,
    /// Exact preregistered OAuth client allowed to act for this subject.
    /// Required even for fixtures; missing client bindings fail closed.
...
pub struct VerifiedAccess {
    ...
    not_before: i64,
    jti: String,
    bearer: Option<Arc<SecretBearer>>,
}
...
pub enum AuthError {
    ...
    InvalidToken,
    #[error("tenant, subject or client is not admitted to this metrics resource")]
    NotAdmitted,
...
pub struct AuthVerifier {
    ...
    admissions: BTreeMap<(TenantId, String), (String, BTreeSet<String>)>,
...
impl AuthVerifier {
    pub fn new(config: AuthConfig) -> Result<Self, AuthError> {
        ...
        for admission in &config.admissions {
            if &admission.tenant_id != tenant
                ...
                || !valid_identifier(&admission.client_id, 256)
                ...
                || admissions
                    .insert(
                        ...
                        (admission.client_id.clone(), admission.scopes.clone()),
                    )
                    .is_some()
            {
                return Err(AuthError::InvalidConfig(
                    "admissions must name unique subjects in one tenant with exact clients and concrete metric scopes",
                ));
            }
        }
    ...
    pub fn verify_bearer_at(
        ...
        let access = self.decode_bearer(authorization)?;
        self.check_access_at(&access, now)?;
        Ok(access)
    }

    /// Authenticate only the original token for self-revocation. This returns
    /// metadata, never an authority object. Lost membership, role, admission,
    /// expiry or prior revocation must not prevent withdrawing that token.
    /// Signature, issuer, audience, tenant and bounded signed-claim shape still
    /// apply. A caller cannot choose a different JTI to revoke.
    pub fn revocable_bearer(
        &self,
        authorization: Option<&str>,
    ) -> Result<ResourceAccess, AuthError> {
        let access = self.decode_bearer(authorization)?;
        let tenant = self
            .admissions
            .keys()
    ...
    fn decode_bearer(&self, authorization: Option<&str>) -> Result<VerifiedAccess, AuthError> {
    ...
    pub fn check_access_at(&self, access: &VerifiedAccess, now: i64) -> Result<(), AuthError> {
        ...
        let (client_id, admitted_scopes) = self
            .admissions
        ...
        if !access.scopes.is_subset(admitted_scopes) {
...
mod tests {
    ...
    #[test]
    fn wrong_audience_issuer_tenant_subject_client_or_scope_is_rejected() {
        let verifier = AuthVerifier::new(config()).unwrap();
        for (field, value) in [
            ...
            ("client_id", json!("different-client")),
            ("scope", json!("metrics:*")),
    ...
    #[test]
    fn admissions_require_explicit_valid_client_bindings() {
        let mut unbound = serde_json::to_value(config()).unwrap();
        unbound["admissions"][0]
            .as_object_mut()
            .unwrap()
            .remove("client_id");
        assert!(serde_json::from_value::<AuthConfig>(unbound).is_err());
        for client_id in ["", "client with spaces", "client\nname"] {
            let mut changed = config();
            changed.admissions[0].client_id = client_id.into();
            assert!(AuthVerifier::new(changed).is_err());
        }
        let mut changed = config();
        changed.admissions[0].client_id = "a".repeat(257);
        assert!(AuthVerifier::new(changed).is_err());
    }
    ...
    #[test]
    fn self_revocation_authenticates_original_token_without_restoring_access() {
        let token = signed(&claims());
        let verifier = AuthVerifier::new(config()).unwrap();
        verifier.revoke_jti("fixture-token-01").unwrap();
        assert_eq!(
            verifier.revocable_bearer(Some(&token)).unwrap().jti,
            "fixture-token-01"
        );
        assert_eq!(
            verifier.verify_bearer_at(Some(&token), NOW).unwrap_err(),
            AuthError::Revoked
        );
        assert_eq!(
            verifier
                .verify_bearer_at(Some(&token), NOW + 600)
                .unwrap_err(),
            AuthError::InvalidToken
        );

        // Changed trusted admission cannot prevent withdrawing a token that
        // was previously issued for this exact tenant and resource.
        let mut changed = config();
        changed.admissions[0].subject = "replacement-user".into();
        changed.admissions[0].client_id = "replacement-client".into();
        let verifier = AuthVerifier::new(changed).unwrap();
        let metadata: ResourceAccess = verifier.revocable_bearer(Some(&token)).unwrap();
        assert_eq!(metadata.jti, "fixture-token-01");
        assert_eq!(metadata.subject, "customer-a-user");
        assert_eq!(
            verifier.verify_bearer_at(Some(&token), NOW).unwrap_err(),
        ...
        for (field, value) in [
            ("iss", json!("https://foreign-issuer.example")),
            ("aud", json!("https://foreign-resource.example/mcp")),
            ("tenant_id", json!("customer-b")),
            ("jti", json!("")),
            ("exp", json!(NOW + 901)),
            ("scope", json!("metrics:*")),
            ...
            changed[field] = value;
            assert!(
                verifier.revocable_bearer(Some(&signed(&changed))).is_err(),
                "accepted invalid {field}"
            );
        }
        ...
        assert!(
            verifier
                .revocable_bearer(Some(std::str::from_utf8(&tampered).unwrap()))
                .is_err()
        );
    }
...
/// Original bearer storage is deliberately redacted from all Debug output.
struct SecretBearer(zeroize::Zeroizing<String>);
impl std::fmt::Debug for SecretBearer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("[redacted resource bearer]")
    }
}

/// Gateway-only credential. It grants token checks and self-revocation, never
/// delegation, approvals, credential resolution, or arbitrary broker methods.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct BrokerClientConfig {
    ...
    pub broker_uid: u32,
    pub binding: crate::tenant::TenantBinding,
}
...
impl ResourceEnvelope {
    pub fn sign(request: ResourceRequest, key: &[u8]) -> Result<Self, AuthError> {
        use hmac::{Hmac, Mac};
        let mut mac =
            Hmac::<sha2::Sha256>::new_from_slice(key).map_err(|_| AuthError::Unavailable)?;
        mac.update(&serde_json::to_vec(&request).map_err(|_| AuthError::Unavailable)?);
        Ok(Self {
            request,
            mac: URL_SAFE_NO_PAD.encode(mac.finalize().into_bytes()),
        })
    }
    pub fn authenticate(&self, key: &[u8]) -> Result<(), AuthError> {
        use hmac::{Hmac, Mac};
        let mut mac =
            Hmac::<sha2::Sha256>::new_from_slice(key).map_err(|_| AuthError::Unavailable)?;
        mac.update(&serde_json::to_vec(&self.request).map_err(|_| AuthError::Unavailable)?);
        mac.verify_slice(
            &URL_SAFE_NO_PAD
                .decode(&self.mac)
...
impl From<&VerifiedAccess> for ResourceAccess {
    fn from(access: &VerifiedAccess) -> Self {
        Self {
...
impl ResourceResponse {
    pub fn denied(binding: crate::tenant::TenantBinding, error: AuthError) -> Self {
        let code = match error {
            AuthError::MissingBearer => "missing_bearer",
            AuthError::InvalidToken => "invalid_token",
            AuthError::NotAdmitted => "not_admitted",
...
/// A synchronous, bounded local authorization call. No authorization decision
/// is cached: each check sends the original token to the broker. The peer UID
/// is checked before sending the bearer or the separate resource credential.
...
impl BrokerClient {
    pub fn new(
        ...
        config
            .binding
            .validate()
            .map_err(|_| AuthError::InvalidConfig("invalid broker binding"))?;
        if !config.socket_path.is_absolute() || !config.credential_file.is_absolute() {
            return Err(AuthError::InvalidConfig(
                "resource socket and credential paths must be absolute",
            ));
        }
        let mut file = std::fs::OpenOptions::new()
            ...
            .map_err(|_| AuthError::Unavailable)?;
        let metadata = file.metadata().map_err(|_| AuthError::Unavailable)?;
        if !metadata.is_file() || metadata.mode() & 0o027 != 0 || metadata.len() != 32 {
            return Err(AuthError::InvalidConfig(
                "resource credential must be a private 32-byte file",
            ));
        }
    ...
    fn call(&self, authorization: &str, revoke: bool) -> Result<ResourceAccess, AuthError> {
        ...
        if !metadata.file_type().is_socket()
            || metadata.uid() != self.config.broker_uid
        ...
        let socket = socket2::Socket::new(socket2::Domain::UNIX, socket2::Type::STREAM, None)
            .map_err(|_| AuthError::Unavailable)?;
        let address = socket2::SockAddr::unix(&self.config.socket_path)
            .map_err(|_| AuthError::Unavailable)?;
        socket
            .connect_timeout(&address, std::time::Duration::from_secs(2))
            .map_err(|_| AuthError::Unavailable)?;
        let mut stream: UnixStream = socket.into();
        if crate::peer::peer_info_from_fd(stream.as_raw_fd())
            .map_err(|_| AuthError::Unavailable)?
            .uid
        ...
        stream
            ...
            .map_err(|_| AuthError::Unavailable)?;
        let envelope = ResourceEnvelope::sign(
            ResourceRequest {
        ...
        let bytes = zeroize::Zeroizing::new(
            serde_json::to_vec(&envelope).map_err(|_| AuthError::Unavailable)?,
        );
        stream
        ...
        self.config
            .binding
            .require_same(&response.binding)
            .map_err(|_| AuthError::NotAdmitted)?;
        if let Some(error) = response.error {
            return Err(match error.as_str() {
                "missing_bearer" => AuthError::MissingBearer,
                "invalid_token" => AuthError::InvalidToken,
                "not_admitted" => AuthError::NotAdmitted,
                "insufficient_scope" => AuthError::InsufficientScope,
                "revoked" => AuthError::Revoked,
                _ => AuthError::Unavailable,
            });
        }
        let access = response.access.ok_or(AuthError::Unavailable)?;
        if access.tenant_id != self.config.binding.tenant_id
            || (!revoke && now()? >= access.expires_at)
        {
            return Err(AuthError::InvalidToken);
        }
        Ok(access)
    ...
    pub fn verify_bearer(&self, authorization: Option<&str>) -> Result<VerifiedAccess, AuthError> {
        let bearer = authorization.ok_or(AuthError::MissingBearer)?;
        let access = self.call(bearer, false)?;
        Ok(VerifiedAccess {
            authority: self.authority.clone(),
            ...
            not_before: 0,
            jti: access.jti,
            bearer: Some(Arc::new(SecretBearer(zeroize::Zeroizing::new(
                bearer.into(),
            )))),
        })
    }
    pub fn check_access(&self, access: &VerifiedAccess) -> Result<(), AuthError> {
        self.check(access, false)
    }
    pub fn revoke_access(&self, access: &VerifiedAccess) -> Result<(), AuthError> {
        self.check(access, true)
    }
    /// Denial-only operation: no VerifiedAccess can be manufactured from this.
    pub fn revoke_bearer(&self, authorization: Option<&str>) -> Result<ResourceAccess, AuthError> {
        self.call(authorization.ok_or(AuthError::MissingBearer)?, true)
    }
    fn check(&self, access: &VerifiedAccess, revoke: bool) -> Result<(), AuthError> {
        if !Arc::ptr_eq(&self.authority, &access.authority) {
            return Err(AuthError::NotAdmitted);
        }
        let token = access.bearer.as_ref().ok_or(AuthError::NotAdmitted)?;
        if ResourceAccess::from(access) != self.call(&token.0, revoke)? {
            return Err(AuthError::NotAdmitted);
        }
        Ok(())
    }
...
mod transport_tests {
    ...
    fn request() -> ResourceRequest {
        ResourceRequest {
            binding: crate::tenant::TenantBinding::new(
    ...
    #[test]
    fn resource_credential_authenticates_every_request_field() {
        for field in 0..6 {
            let mut envelope = ResourceEnvelope::sign(request(), &[7; 32]).unwrap();
            envelope.authenticate(&[7; 32]).unwrap();
            assert_eq!(envelope.authenticate(&[8; 32]), Err(AuthError::NotAdmitted));
            match field {
                0 => envelope.request.binding.broker_id = uuid::Uuid::new_v4(),
                1 => envelope.request.binding.tenant_id = TenantId::parse("other").unwrap(),
                2 => envelope.request.authorization = "Bearer other-token".into(),
                3 => envelope.request.issuer = "https://other.example".into(),
                4 => envelope.request.audience = "https://other.example/mcp".into(),
                _ => envelope.request.revoke = true,
            }
            assert_eq!(envelope.authenticate(&[7; 32]), Err(AuthError::NotAdmitted));
        }
    }
    #[test]
    fn bearer_debug_is_redacted() {
        let bearer = SecretBearer(zeroize::Zeroizing::new("sensitive-token".into()));
        assert!(!format!("{bearer:?}").contains("sensitive-token"));
    }
}
