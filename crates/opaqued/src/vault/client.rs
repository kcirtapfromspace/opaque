...
mod tests {
    ...
    async fn renew_lease_posts_expected_payload() {
        ...
        let lease = client
            ...
            .await
            .unwrap();
        assert_eq!(lease.lease_id, "database/creds/readonly/a1");
        assert_eq!(lease.lease_duration_secs, 90);
        assert!(lease.renewable);
    }

    #[tokio::test]
    async fn renew_lease_maps_error_statuses() {
        let server = MockServer::start().await;
        let client = VaultClient::with_base_url(server.uri());
        ...
        let bad_request = client
            ...
            .await
            .unwrap_err();
        assert!(matches!(bad_request, VaultApiError::BadRequest));
        ...
        let not_found = client
            ...
            .await
            .unwrap_err();
        assert!(matches!(not_found, VaultApiError::NotFound(_)));
    }
}
