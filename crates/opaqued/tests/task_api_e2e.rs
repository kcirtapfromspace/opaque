...
async fn mock_github() -> MockServer {
    ...
    Mock::given(method("PUT"))
        .and(path(
            "/repos/acme/task-widgets/actions/secrets/TASK_SECRET",
        ))
        .and(header("authorization", format!("Bearer {TEST_PAT}")))
...
async fn task_plan_run_get_list_revoke_end_to_end() {
    ...
    let planned = daemon
        .call("task_plan", json!({"manifest": manifest("publish task secret")}))
        .await;
    ...
    let task_id2 = planned2["result"]["task"]["id"]
        ...
        .to_string();
    assert_ne!(task_id, task_id2, "each task_plan call must mint a fresh id");
    ...
    let got = daemon
        .call("task_get", json!({"task_id": task_id}))
        .await;
    ok(&got, "task_get");
    ...
    let run_after_revoke = daemon
        .call("task_run", json!({"task_id": task_id2}))
        .await;
    assert!(
