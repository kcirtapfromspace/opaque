...
//! pass that result only to the inline (set-secret) path. `task_api` stays in
//! `opaqued` (destined for a future `opaque-bounded-work` extraction) and
//! cannot be a dependency of this crate without creating a cycle (`opaqued`
//! depends on `opaque-providers`), so the already-verified workspace is now a
//! parameter threaded through all three paths instead — same verified value,
//! one verification instead of two.
//!
//! Kept crate-private helper naming (`truncate_for_error`) intentionally
//! duplicated rather than shared with `opaqued::main.rs`'s copy: it is a
//! trivial, stateless, five-line pure function used in several other RPC
//! arms that stay in `opaqued` (`gitlab`, `onepassword`, `bitwarden`), so
//! there is no single non-`opaqued` home for it that wouldn't be an
//! over-large seam for this move.
...
/// Crate-private duplicate of `opaqued::main.rs`'s helper of the same name —
/// see the module doc for why this small function isn't shared.
...
/// `task_api::verified_workspace`) for every `"github"`-method request.
pub async fn handle_github_rpc(
    ...
    if action == "list_secrets" {
        return handle_github_list_secrets(req, state, identity, client_type, principal_ctx, workspace)
            .await;
    }
